...
const getUserData = async (uid) => {
  ...
  if (docSnap.exists()) {
    ...
    if (fullNameSection)
      fullNameSection.innerHTML =
        docSnap.data().fullName + " " + docSnap.data().lastName;
    localStorage.setItem("user", docSnap.data());
    localStorage.setItem("email", docSnap.data().email);

    if (firstName) {
      firstName.value = docSnap.data().fullName.split(" ");
...
// profileBtn.addEventListener('click', proBtn);